...
class FileToolsMixin:
    ...
    def _shortcut_dlg(self):
        ...
        f1 = tk.Frame(win, bg=self.colors['light']); f1.pack(pady=10)
        tk.Label(f1, text="目标:", bg=self.colors['light']).grid(row=0, col=0)
        fp = tk.StringVar(); tk.Entry(f1, textvariable=fp, width=45).grid(row=0, col=1, padx=5)
        tk.Button(f1, text="浏览", command=lambda: fp.set(filedialog.askopenfilename() or fp.get())).grid(row=0, col=2)
        tk.Label(f1, text="名称:", bg=self.colors['light']).grid(row=1, col=0, pady=8)
        nv = tk.StringVar(); tk.Entry(f1, textvariable=nv, width=30).grid(row=1, col=1, padx=5, sticky="w")
